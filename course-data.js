...
window.COURSE = [
  ...
  topics:[
   ...
   },
   {
    ...
    quiz:[
     ...
     {type:"mcq", q:"Construct the causal chain of aging — put these in the correct order: DNA damage · imperfect DNA repair · mutation accumulation · cellular dysfunction · aging phenotypes.", opts:["DNA damage → imperfect DNA repair → mutation accumulation → cellular dysfunction → aging phenotypes","DNA damage → mutation accumulation → imperfect DNA repair → aging phenotypes → cellular dysfunction","Mutation accumulation → DNA damage → imperfect DNA repair → cellular dysfunction → aging phenotypes","Imperfect DNA repair → DNA damage → cellular dysfunction → mutation accumulation → aging phenotypes"], a:0,
      exp:"Order: (1) DNA damage → (2) imperfect DNA repair fails to fully fix it → (3) mutations accumulate → (4) cellular dysfunction → (5) aging phenotypes."},
     {type:"mcq", q:"ROS oxidize guanine to 8-oxo-deoxyguanosine (8-oxodG). Why can this lesion lead to a PERMANENT mutation after DNA replication?", opts:["8-oxodG makes DNA polymerase stop replication permanently, causing chromosome loss","8-oxodG can mispair with adenine during replication, ultimately converting an original G–C base pair into a T–A base pair after subsequent rounds of replication","8-oxodG is recognized as thymine and converts a G–C pair directly into an A–T pair in a single replication cycle","8-oxodG pairs with cytosine, causing a change in the DNA sequence"], a:1,
      exp:"8-oxodG mispairs with ADENINE (due to its altered structure, not because it is 'recognized as thymine'). After a further round of replication that A pairs with T, fixing the original G–C as T–A. The permanent mutation requires SUBSEQUENT rounds — not a single cycle."},
     {type:"mcq", q:"A base mismatch is due to:", opts:["The cutting out of a few nucleotides","Incorrect pairing of bases","Substitution of a base with another one","Nucleotide (base) damage"], a:1,
      exp:"A mismatch = an incorrectly paired base (e.g., a replication error placing the wrong base opposite the template). Mismatch repair (MMR) corrects it."},
     {type:"mcq", q:"Which DNA repair pathway is activated after DNA damage that DISTORTS the double-stranded helix (bulky lesions)?", opts:["Base excision repair","Non-homologous end joining","Mismatch repair","Nucleotide excision repair"], a:3,
      exp:"NER removes bulky, helix-distorting lesions (e.g., UV thymine dimers). BER fixes small/oxidative lesions; MMR fixes mismatches; NHEJ rejoins double-strand breaks."},
     {type:"mcq", q:"For homologous recombination (HR) to repair a double-strand break, the cell requires:", opts:["A similar (homologous) copy of undamaged DNA as a template","Base excision repair to occur first","The broken ends to simply be fused together","No template at all"], a:0,
      exp:"HR is template-based — it uses a homologous undamaged copy (the sister chromatid), so it's accurate and works in S/G2. NHEJ is the no-template 'fuse the ends' pathway (error-prone)."},
     {type:"mcq", q:"Match the repair systems to the damage they fix. Which set of pairings is entirely CORRECT?", opts:["BER → abnormal/damaged bases; NER → helix-distorting lesions; MMR → replication errors (mispaired bases); HR & NHEJ → double-strand breaks","BER → double-strand breaks; NER → mismatches; MMR → abnormal bases; HR → helix distortion","BER → replication errors; NER → double-strand breaks; MMR → abnormal bases; NHEJ → mismatches","BER → helix distortion; NER → abnormal bases; MMR → double-strand breaks; HR → replication errors"], a:0,
      exp:"BER = abnormal single bases (oxidative/deamination); NER = bulky helix-distorting lesions; MMR = replication errors/mispaired bases; BOTH HR and NHEJ repair double-strand breaks (HR uses a template, NHEJ fuses the ends)."},
     {type:"mcq", q:"Tobacco smoke primarily causes which type of DNA damage?", opts:["Chemical changes to nucleotides (base adducts/modifications)","Double-strand breaks","Single-strand breaks","Homologous recombination"], a:0,
      exp:"Tobacco-smoke carcinogens chemically modify bases (bulky adducts) → chemical changes to nucleotides, repaired mainly by NER. ('Homologous recombination' is a repair pathway, not a type of damage — a distractor.)"},
     {type:"mcq", q:"When do base mismatches occur?", opts:["After DNA replication","During transcription","During DNA replication","During interphase"], a:2,
      exp:"Mismatches arise during DNA replication when polymerase inserts the wrong base opposite the template; mismatch repair (MMR) then corrects them."},
     {type:"mcq", q:"Why might chronic DNA damage contribute to the age-associated decline in cellular NAD⁺ levels?", opts:["DNA damage causes NAD⁺ to be incorporated directly into DNA","DNA damage activates PARP enzymes, which consume NAD⁺ during DNA-repair signaling","DNA damage converts NAD⁺ into reactive oxygen species","DNA damage prevents cells from synthesizing ATP"], a:1,
      exp:"PARPs detect DNA breaks and consume NAD⁺ as they signal for repair. Chronic damage over-activates PARPs, draining the NAD⁺ pool (which sirtuins also depend on) → the proposed damage→PARP→NAD⁺↓ aging cycle."},
     {type:"mcq", q:"During replication, DNA polymerase occasionally inserts a wrong nucleotide. How does its 3′→5′ exonuclease (proofreading) activity maintain fidelity?", opts:["It joins Okazaki fragments after synthesis is complete","It removes the incorrectly paired nucleotide from the growing strand so polymerase can insert the correct one","It synthesizes DNA in the 3′→5′ direction to replace damaged nucleotides","It removes RNA primers before replication continues"], a:1,
      exp:"Proofreading exonuclease excises the mispaired base it just added, letting polymerase re-insert the correct one (error rate < 10⁻⁹). Trap: polymerase SYNTHESIZES 5′→3′; the exonuclease REMOVES nucleotides, it doesn't synthesize."},
     {type:"mcq", q:"Two cell populations are compared: one has normal base excision repair, the other lacks a key enzyme needed to repair apurinic (AP) sites. What is most likely observed in the repair-deficient cells?", opts:["Increased mutation frequency due to persistence of apurinic sites generated by spontaneous depurination","Elimination of all oxidative DNA damage","Lower mutation frequency due to reduced DNA replication","Increased DNA repair efficiency"], a:0,
      exp:"BER normally fixes AP sites. Without it, spontaneous depurination (~thousands/cell/day) leaves AP sites that persist; during replication a random base is inserted opposite them → more mutations. A repair deficiency can't eliminate damage or improve efficiency."}
    ]
   },
   {
 ...
 /* ============================ LECTURE 4 ============================ */
 {
  ...
  topics:[
   ...
   {
    id:"L4Q", title:"📝 Practice quiz (from class) — Epigenetics",
    sub:"The class Epigenetics practice quiz, with the CORRECT answers keyed (incl. the ones that are easy to miss).",
    slides:[
     {h:"📝 Practice quiz", imgs:[], html:`<div class="callout tip">The exact questions from the class <b>Epigenetics</b> practice quiz. The matching item is split into individual true/false calls so you drill each scenario. Watch the traps: acetylation = ACTIVE, and genetic mutations / natural selection are NOT epigenetic.</div>`}
    ],
    quiz:[
     {type:"mcq", q:"What is true regarding the role of epigenetics in biological processes?", opts:["Epigenetic modifications always lead to diseases","Epigenetic modifications are irrelevant for the functioning of a cell","Epigenetic modifications are rare","Epigenetic modifications are a normal process in the development of organisms"], a:3,
      exp:"Epigenetics is essential and normal — it drives development and cell differentiation, not just disease."},
     {type:"tf", q:"SCENARIO: Nutrient-restricted rats had a greater risk of infection than well-fed rats. — Is this LIKELY due to an epigenetic modification?", a:false,
      exp:"NOT epigenetic — a direct physiological/immune effect of malnutrition in the same animals, not a heritable expression change."},
     {type:"tf", q:"SCENARIO: Offspring of high-fat-diet rats were themselves fed a low-fat diet, stayed normal weight, but still developed diabetes. — Epigenetic?", a:true,
      exp:"Epigenetic (transgenerational): the parental diet altered heritable gene expression, so offspring get diabetes despite their own healthy diet."},
     {type:"tf", q:"SCENARIO: In agouti mice, a methyl-rich maternal diet methylates the agouti allele, turning yellow-coat offspring brown. — Epigenetic?", a:true,
      exp:"Classic epigenetic example — DNA methylation (not sequence) changes coat color."},
     {type:"tf", q:"SCENARIO: A BRCA1 gene mutation is inherited; carriers develop cancer and non-carriers do not. — Epigenetic?", a:false,
      exp:"NOT epigenetic — a BRCA1 mutation is a change in the DNA SEQUENCE (genetic), not an epigenetic mark."},
     {type:"tf", q:"SCENARIO: Pluripotency regions of stem cells (euchromatin) become heterochromatin as the cells differentiate. — Epigenetic?", a:true,
      exp:"Epigenetic — chromatin remodeling switches gene sets off during differentiation, with no DNA sequence change."},
     {type:"tf", q:"SCENARIO: A Husky is born with heterochromia (one blue, one brown eye), like its mother. — Epigenetic?", a:false,
      exp:"NOT epigenetic — inherited eye-color/heterochromia is genetic/developmental pigmentation, not a methylation-based expression change."},
     {type:"tf", q:"SCENARIO: People with a history of childhood abuse show more methylation of the glucocorticoid-receptor gene. — Epigenetic?", a:true,
      exp:"Epigenetic — an environmental experience adds DNA methylation that alters gene expression."},
     {type:"tf", q:"SCENARIO: After a hurricane kills most lizards, the next seven generations are all red instead of red/yellow. — Epigenetic?", a:false,
      exp:"NOT epigenetic — this is natural SELECTION acting on existing genetic variation, not an epigenetic change."},
     {type:"tf", q:"SCENARIO: Dioxin-exposed pregnant rats have offspring with kidney disease that persists for three generations, even in lines not directly exposed. — Epigenetic?", a:true,
      exp:"Epigenetic (transgenerational) — a toxin induced a heritable expression change passed down without further exposure."},
     {type:"mcq", q:"Why is it important to train an epigenetic-clock algorithm using DNA methylation data from a DIVERSE group of individuals?", opts:["A diverse dataset ensures all individuals will have the same biological age","A diverse dataset reduces the need for machine learning by making the patterns identical","A diverse dataset helps the algorithm learn aging patterns that are more likely to generalize across different populations","A diverse dataset increases the DNA mutation rate and improves prediction accuracy"], a:2,
      exp:"Diverse training data lets the model generalize across populations instead of overfitting one group ('garbage in, garbage out')."},
     {type:"mcq", q:"You study a histone complex with an unusually HIGH level of acetylation. What can you conclude about the DNA bound to it?", opts:["It is most likely telomeric","It is most likely heterochromatin","It is most likely derived from a condensed Barr body","It contains actively transcribed genes"], a:3,
      exp:"High acetylation = OPEN chromatin (euchromatin) = ACTIVE transcription. Heterochromatin/Barr bodies are LOW-acetylation and silenced — the opposite."},
     {type:"mcq", q:"What is the purpose of training an AI model on DNA methylation profiles from many individuals?", opts:["To measure how many chromosomes a person has","To predict a person's biological age based on DNA methylation patterns","To identify all genes present in an individual's genome","To modify a person's DNA sequence to slow aging"], a:1,
      exp:"The model learns methylation patterns that predict biological age — it doesn't count chromosomes or edit DNA."},
     {type:"mcq", q:"How do epigenetic clocks estimate biological age?", opts:["By counting the number of cell divisions in tissues","By detecting DNA methylation patterns that statistically correlate with aging","By directly measuring the amount of cellular damage accumulated over time","By directly measuring how much time has passed since birth"], a:1,
      exp:"Clocks are predictive models built on age-CORRELATED methylation — they don't directly measure damage or elapsed time."},
     {type:"mcq", q:"What is true about epigenetic marks and cell division?", opts:["Epigenetic marks of mother cells are generally maintained in daughter cells; however, they are susceptible to environmental influences","Epigenetic marks of daughter cells are completely different from those of mother cells","Epigenetic marks in mother and daughter cells are always identical and cannot be influenced by the environment","Epigenetic marks never survive cell divisions"], a:0,
      exp:"Marks are generally inherited by daughter cells (so cell identity is maintained) but remain modifiable by the environment — not rigidly fixed, and not erased."},
     {type:"mcq", q:"Epigenetics describes the interaction of DNA with:", opts:["Photons","Contaminants","Cells","Molecules inside cells"], a:3,
      exp:"Epigenetics involves molecules (methyl groups, histone-modifying enzymes, etc.) interacting with DNA inside the cell."},
     {type:"mcq", q:"The epigenome is:", opts:["The part of the genome relevant for epidemiological studies","The set of all mutations that occur in epidermic cells","The set of all epigenetic modifications in the genome of a cell","The set of all epistatic mutations in a cell"], a:2,
      exp:"The epigenome = the complete set of epigenetic modifications (methylation, histone marks, etc.) across a cell's genome."},
     {type:"mcq", q:"What can you conclude from nuclear transplantation (cloning) in cattle?", opts:["Only embryonic cells contain the full set of genetic instructions required for development","The nucleus of a differentiated cell retains the complete genetic information needed to produce an entire organism","Cloned calves are genetically different because they develop independently","Differentiated cells lose genes that are not needed for their specialized function"], a:1,
      exp:"Cloning from a differentiated cell's nucleus shows it keeps the FULL genome — differentiation is epigenetic (which genes are ON), not loss of genes."},
 ...
 /* ============================ LECTURE 5 ============================ */
 {
  ...
  topics:[
   ...
   {
    id:"L5Q", title:"📝 Practice quiz (from class) — Proteostasis",
    sub:"The class Proteostasis practice quiz, with the CORRECT answers keyed (incl. the PROTAC trap).",
    slides:[
     {h:"📝 Practice quiz", imgs:[], html:`<div class="callout tip">The exact questions from the class <b>Proteostasis</b> practice quiz. Core ideas: chaperones FOLD (they don't traffic); mono-ubiquitin = regulatory, poly-ubiquitin = degradation; PROTAC = degradation via an E3 ligase (not translation blocking).</div>`}
    ],
    quiz:[
     {type:"mcq", q:"Histone chaperones assemble and distribute histones onto newly synthesized DNA during replication. What is the most likely outcome of mutations in a histone-chaperone gene?", opts:["Increased mutation rate because histone chaperones directly repair damaged DNA","Failure to replicate the DNA sequence, preventing chromosome duplication","Loss of proper inheritance of histone modifications, leading to altered chromatin states and abnormal gene expression across cell generations","Permanent elimination of all DNA methylation marks from the genome"], a:2,
      exp:"Histone chaperones guide histone deposition so chromatin (and its modification patterns) is faithfully inherited. Mutate them → chromatin states aren't properly passed on → abnormal gene expression across generations."},
     {type:"mcq", q:"In aging cells a histone protein becomes increasingly polyubiquitinated and degraded. Most likely epigenetic consequence?", opts:["Increased DNA replication speed because nucleosomes become more stable","Elimination of all DNA methylation marks across the genome","Increased chromatin remodeling and altered gene expression due to changes in histone abundance","Permanent mutation of the DNA sequence causing inherited genetic changes"], a:2,
      exp:"Degrading histones changes histone abundance → altered nucleosome/chromatin structure → shifted gene expression (an epigenetic, not sequence, change)."},
     {type:"mcq", q:"Why might it be an evolutionary advantage for chaperones to physically interact with the ribosome?", opts:["A chaperone that engages newly synthesized proteins earlier reduces improper folding and improves cellular fitness","The chaperones replace the need for ribosomes by directly synthesizing proteins","The chaperones permanently lock proteins into their final structure, preventing future modifications","These chaperones directly increase the speed of translation"], a:0,
      exp:"Ribosome-associated chaperones catch the nascent chain as it emerges, preventing misfolding/aggregation early → better proteostasis and fitness."},
     {type:"mcq", q:"A mutation prevents histone MONO-ubiquitination but not polyubiquitination. Most likely effect on epigenetic regulation?", opts:["Increased DNA mutation rate due to failure of DNA replication","Reduced ability to dynamically regulate chromatin structure and gene expression","Permanent silencing of all genes","Complete loss of histone proteins through proteasomal degradation"], a:1,
      exp:"Mono-ubiquitin on histones is a REGULATORY mark (dynamic chromatin control). Lose it → impaired dynamic regulation of chromatin/gene expression. (Poly-ubiquitin, which still works here, is the degradation signal.)"},
     {type:"mcq", q:"A PROTAC targeting an overactive kinase lowers the kinase PROTEIN level even though its gene is still transcribed normally. Best explanation?", opts:["The PROTAC blocks ribosomes from translating the kinase mRNA into protein","The PROTAC simultaneously binds the target protein and an E3 ubiquitin ligase, promoting ubiquitination and proteasomal degradation of the target","The PROTAC enters the nucleus and permanently silences transcription of the kinase gene","The PROTAC competes with ATP and inhibits the kinase's activity without affecting its abundance"], a:1,
      exp:"PROTAC = chimera linking target + E3 ligase → ubiquitination → proteasomal DEGRADATION. Protein drops although it's still transcribed/translated — it's destroyed after the fact, not blocked at translation."},
     {type:"mcq", q:"A drug inhibits ATP binding to HSP90 (without affecting protein synthesis). Cells show reduced growth signaling and accumulation of unstable signaling proteins. Best explanation?", opts:["HSP90 normally degrades signaling proteins, so inhibition makes them unstable","HSP90 requires ATP-driven conformational changes to stabilize and mature client proteins, so inhibition causes their loss of function and degradation","HSP90 is required for ribosomes to synthesize proteins, so ATP inhibition stops translation","HSP90 directly transcribes growth-related genes, so blocking ATP prevents mRNA production"], a:1,
      exp:"HSP90 uses an ATP-driven cycle to mature/stabilize client signaling proteins; block ATP → clients can't fold/stabilize → they lose function and are degraded."},
     {type:"mcq", q:"Which mechanism best describes how TRiC promotes correct protein folding?", opts:["It covalently attaches proteins to membrane lipids","It labels misfolded proteins with ubiquitin so they are always degraded instead of folded","It fully unfolds substrates and releases them for random refolding without energy","It binds partially folded substrates, uses ATP hydrolysis to close its lid, and creates a protected chamber where the protein folds into its native shape"], a:3,
      exp:"TRiC (CCT) is an isolation chamber: substrate sequestered inside, ATP closes the lid, and it folds shielded from aggregation, then is released."},
     {type:"mcq", q:"Which of the following is NOT a function of chaperone proteins?", opts:["Help refold proteins that became partially unfolded due to stress","Guide appropriately folded proteins to their correct cellular location","Prevent misfolding and aggregation of unfolded proteins","Assist newly synthesized proteins in achieving their correct tertiary structure"], a:1,
      exp:"Chaperones FOLD and prevent aggregation — they do NOT traffic already-folded proteins to locations (that's handled by signal sequences/transport systems)."},
     {type:"mcq", q:"Which step is the KEY reason a PROTAC can trigger degradation of a target protein?", opts:["It binds the target and prevents its folding, causing it to dissolve","It binds the target on a residue that tags it for phosphorylation","It permanently blocks the active site so the protein is destroyed","It brings the target protein and an E3 ubiquitin ligase together so the target is ubiquitinated and sent to the proteasome"], a:3,
 ...
 /* ============================ LECTURE 7 ============================ */
 {
  ...
  topics:[
   ...
   {
    id:"L7Q", title:"📝 Practice quiz (from class) — Nutrient Sensing",
    sub:"The class Nutrient Sensing practice quiz, with the CORRECT answers keyed (incl. the IGF-1 and autophagy traps).",
    slides:[
     {h:"📝 Practice quiz", imgs:[], html:`<div class="callout tip">The exact questions from the class <b>Nutrient Sensing</b> practice quiz. Key logic: for longevity ACTIVATE AMPK, SUPPRESS mTOR & insulin/IGF-1. IGF-1/mTOR = grow (autophagy OFF); AMPK/FOXO = maintain (autophagy ON).</div>`}
    ],
    quiz:[
     {type:"mcq", q:"Which scenario best illustrates negative-feedback control involving insulin and glucagon in glucose homeostasis?", opts:["Blood glucose falls slightly, insulin secretion increases, and blood glucose falls even more","Blood glucose is normal, both insulin and glucagon remain high, and blood glucose continues to rise","Blood glucose falls during prolonged exercise, glucagon secretion increases, and blood glucose is restored toward normal","Blood glucose rises after a meal, insulin secretion decreases, and blood glucose rises further"], a:2,
      exp:"Negative feedback opposes the change and restores the set point: low glucose → glucagon → glucose back toward normal. The other options describe positive feedback or broken control."},
     {type:"mcq", q:"In C. elegans, reducing daf-2 activity dramatically extends lifespan, but the effect disappears if daf-16 is also inactivated. Best-supported conclusion?", opts:["DAF-2 and DAF-16 independently regulate lifespan through unrelated pathways","DAF-16 shortens lifespan and normally suppresses the effects of DAF-2","Reduced DAF-2 signaling extends lifespan by allowing DAF-16 to activate genes involved in stress resistance and longevity","DAF-2 repairs age-related DNA damage, while DAF-16 controls metabolism"], a:2,
      exp:"DAF-2 (insulin/IGF-1 receptor) normally inhibits DAF-16 (FOXO). Lose DAF-2 → FOXO is freed to turn on stress-resistance/longevity genes. Remove DAF-16 too → no benefit."},
     {type:"mcq", q:"Put the steps of glucose homeostasis in order for when blood glucose INCREASES (after a meal):", opts:["Insulin secreted (β-cells) → glucose taken into cells/liver as glycogen → blood glucose decreases → glucagon secreted (α-cells) → glycogen broken down, glucose released","Glucagon secreted → glycogen broken down → blood glucose rises → insulin secreted → glucose stored","Insulin secreted → glycogen broken down → blood glucose rises → glucagon secreted → glucose stored","Blood glucose decreases → glucagon secreted → insulin secreted → glucose stored → glycogen broken down"], a:0,
      exp:"High glucose → (1) insulin from β-cells → (2) glucose into cells/liver as glycogen → (3) blood glucose falls → (4) glucagon from α-cells → (5) glycogen broken down, glucose released (the loop's other half)."},
     {type:"mcq", q:"What would be the impact of CHRONICALLY HIGH IGF-1 signaling?", opts:["It would enhance stress resistance and autophagy while inhibiting cell survival and repair","It would suppress development and injury repair","It would increase development and the clearance of cellular debris","It would promote growth while reducing stress resistance and autophagy"], a:3,
      exp:"IGF-1 is pro-growth: it drives mTOR (which blocks autophagy) and inhibits FOXO (reducing stress resistance). So chronic IGF-1 = growth up, autophagy & stress resistance down."},
     {type:"mcq", q:"Which scenario best illustrates a coordinated PRO-LONGEVITY state involving AMPK, mTORC1, and FOXO?", opts:["Low energy activates AMPK, which inhibits mTORC1 and permits nuclear FOXO activity, increasing stress resistance and maintenance","High nutrients and growth factors activate AMPK and mTORC1 simultaneously, driving nuclear exclusion of FOXO and enhanced growth","High insulin/IGF-1 signaling turns on AMPK and FOXO together, promoting growth and stress resistance","Low nutrients suppress AMPK, thereby activating mTORC1 and cytoplasmic FOXO to boost anabolic metabolism"], a:0,
      exp:"Pro-longevity = AMPK ON → mTORC1 OFF → FOXO active in the nucleus → stress resistance & maintenance (autophagy). The others mix the signals incorrectly."},
     {type:"mcq", q:"What is an overall effect of GLP-1 analogs?", opts:["GLP-1 increases hepatic lipid content","GLP-1 speeds up gastric emptying","GLP-1 decreases insulin secretion","GLP-1 increases insulin secretion"], a:3,
      exp:"GLP-1 increases insulin secretion (and reduces appetite, slows gastric emptying, lowers hepatic fat). The other three are the wrong direction."},
     {type:"mcq", q:"What distinguishes type 1 from type 2 diabetes?", opts:["Type 1 develops only in children, type 2 only in adults","Type 1 results primarily from autoimmune destruction of pancreatic β-cells; type 2 is characterized by insulin resistance and often progressive β-cell dysfunction","Type 1 causes low blood glucose, type 2 causes high blood glucose","Type 1 is caused by reduced insulin-receptor activity; type 2 occurs when the pancreas stops producing glucagon"], a:1,
      exp:"Type 1 = autoimmune β-cell destruction → too little insulin. Type 2 = insulin resistance (tissues stop responding) with progressive β-cell dysfunction. Both can occur at various ages."},
     {type:"mcq", q:"Based on insulin/IGF-1 receptor signaling, which intervention would most likely INCREASE autophagy?", opts:["Increased activation of the insulin receptor leading to greater Akt and mTOR signaling","Increased activation of PI3K and Akt signaling","Inhibition of mTOR activity downstream of insulin signaling","Increased phosphorylation of ELK1 to promote proliferation"], a:2,
      exp:"Insulin → IRS → PI3K → Akt → mTOR ⊣ ULK ⊣ autophagy. Activating the receptor/Akt SUPPRESSES autophagy; to INCREASE it you must INHIBIT mTOR."},
     {type:"mcq", q:"How might metformin contribute to increased healthspan and longevity?", opts:["By inhibiting mTOR and decreasing AMPK activity, increasing liver glucose production","By activating AMPK and inhibiting mTOR in the liver, reducing glucose production and serum insulin","By activating NF-κB, increasing inflammation to enhance immune surveillance","By increasing serum insulin and IGF-1 levels to promote tissue growth"], a:1,
      exp:"Metformin acts on mitochondria → activates AMPK and inhibits mTOR, lowering hepatic glucose output and serum insulin — a pro-longevity metabolic shift."},
     {type:"mcq", q:"To promote longevity, which pathways should you ACTIVATE vs SUPPRESS?", opts:["Activate AMPK; suppress mTOR and insulin/IGF-1","Suppress AMPK; activate mTOR and insulin/IGF-1","Activate AMPK, mTOR, and insulin/IGF-1 together","Suppress AMPK and mTOR; activate insulin/IGF-1"], a:0,
      exp:"Activate AMPK (energy sensor, promotes autophagy); suppress mTOR and insulin/IGF-1 (both anabolic and block autophagy)."},
     {type:"mcq", q:"In type 2 diabetes, muscle and fat cells see normal/high insulin but still fail to take up glucose efficiently. Best mechanism?", opts:["Insulin resistance causes excessive GLUT4 insertion, so glucose accumulates inside cells","Pancreatic β-cells stop producing glucagon, preventing glucose entry","Insulin resistance reduces insulin signaling, decreasing GLUT4 translocation to the membrane and limiting glucose uptake","Glucose transporters become constitutively active, causing blood glucose to rise"], a:2,
      exp:"Insulin resistance → impaired signaling → GLUT4 vesicles aren't moved to the membrane → glucose uptake into muscle/fat falls → blood glucose stays high."},
     {type:"mcq", q:"A researcher puts cells in a low-glucose environment and sees AMPK activation. Which response would most likely restore energy balance?", opts:["Increased ATP-consuming pathways (protein synthesis) and decreased glucose uptake","Activation of ATP-producing pathways and suppression of energy-consuming processes","Increased cell proliferation and inhibition of catabolic pathways","Activation of mTOR signaling to promote growth despite low energy"], a:1,
      exp:"AMPK (low-energy sensor) turns ON ATP-producing (catabolic) pathways and turns OFF ATP-consuming (anabolic) ones — including inhibiting mTOR — to restore energy balance."},
     {type:"mcq", q:"In a NUTRIENT-RICH environment, which direct effect on mTORC1 is most accurate, and how does it influence longevity?", opts:["Amino acids and insulin inhibit mTORC1, extending lifespan by reducing protein synthesis","Amino acids and insulin signaling activate mTORC1, generally shortening lifespan by promoting growth over maintenance","Oxygen deprivation directly activates mTORC1, shortening lifespan via oxidative phosphorylation","Glucose scarcity directly activates mTORC1, extending lifespan through enhanced autophagy"], a:1,
      exp:"Abundant amino acids + insulin ACTIVATE mTORC1 → growth/protein synthesis at the expense of maintenance/autophagy → tends to shorten lifespan."}
    ]
   },
   {
  ...
  topics:[
   ...
   },
   {
    ...
    quiz:[
     ...
     {type:"short", img:"slides/L9/slide-23.jpg", q:"A knockout mouse cannot make caspase-9. What defects would you predict in the brains of knockout embryos? (Her slide shows the actual Kuida 1998 data.)", model:"Caspase-9 is the initiator caspase of the intrinsic (mitochondrial) pathway, so developmental apoptosis fails. Neurons that should have been culled survive, producing brain overgrowth and malformation — an enlarged, disorganized brain with excess neurons and ectopic cell masses (in the real experiments, exencephaly and malformed forebrain). It shows apoptosis is required to sculpt normal brain structure.", pts:["Caspase-9 = initiator of intrinsic pathway → apoptosis fails","Excess neurons survive (no culling)","Brain overgrowth/malformation, disorganized structure"]},
     {type:"short", img:"slides/L9/slide-24.jpg", q:"Staurosporine (STS) induces apoptosis. Why does DNA from STS-treated cells appear as a smear on an electrophoresis gel?", model:"Apoptosis activates nucleases that chop the cell's chromosomal DNA into many fragments. Instead of one intact high-molecular-weight band (as in untreated cells), the sample now contains DNA pieces of many different sizes, which migrate across a wide range of the gel and appear as a smear. (Strictly internucleosomal cleavage gives the classic 'ladder'; more extensive/heterogeneous degradation smears it.)", pts:["Apoptotic nucleases fragment the DNA","Many fragment sizes instead of one intact band","Untreated control = single high-MW band; treated = smear/ladder"]},
     {type:"short", img:"slides/L9/slide-25.jpg", q:"Dr. B. wants to increase the size of rats. Based on apoptosis biology, what approach should he pursue?", model:"Reduce developmental apoptosis so more cells survive. Options: increase survival-factor signaling (more growth/survival factor, more PI3K–Akt activity), overexpress anti-apoptotic Bcl-2, or inactivate pro-apoptotic Bax/Bak or caspases. Fewer cells culled during development → more cells retained → larger animal. Caveat: blocking apoptosis also raises cancer risk, since damaged cells would survive too.", pts:["Inhibit apoptosis so more cells survive development","E.g., ↑survival factors/Akt, overexpress Bcl-2, or knock out Bax/Bak/caspases","Trade-off: blocking apoptosis increases cancer risk"]}
    ]
   }
  ]
 },
  ...
  topics:[
   {
    ...
    slides:[
     {h:"The cell-cycle control system", imgs:[{src:"slides/L10/slide-01.jpg", cap:"L10 · Slide 1 — Tadalafil propensity-matched cohort: ASSOCIATED with vs CAUSES lower mortality"},{src:"slides/L10/slide-05.jpg", cap:"L10 · Slide 5 — The cell-division machine: the pedal is the growth signal; a jammed pedal makes cancer"},{src:"slides/L10/slide-03.jpg", cap:"L10 · Slide 3 — We are made from organized, specialized cells"},{src:"slides/L10/slide-04.jpg", cap:"L10 · Slide 4 — A fertilized egg divides into millions of cells"},{src:"slides/L10/slide-07.jpg", cap:"L10 · Slide 7 — Failing a checkpoint: death"},{src:"slides/L10/slide-09.jpg", cap:"L10 · Slide 9 — Which normal cell is most vulnerable to becoming cancerous?"},
    ...
    quiz:[
     {type:"mcq", q:"A propensity-matched cohort of 509,788 men found tadalafil use was linked to 34% lower all-cause mortality. What may researchers conclude?", img:"slides/L10/slide-01.jpg", opts:["Tadalafil is ASSOCIATED with lower mortality after adjusting for measured confounders","Tadalafil CAUSES lower mortality","Tadalafil should be prescribed to everyone for longevity","Nothing can be learned from this study"], a:0,
      exp:"Propensity matching adjusts only for MEASURED confounders — unmeasured ones remain. So the honest conclusion is association, not causation. Note the enormous sample size does NOT fix this: n = 509,788 makes the estimate precise, not causal."},
     {type:"mcq", q:"Why does a huge sample size (509,788) still NOT let you claim tadalafil causes lower mortality?", opts:["Large n improves precision but cannot remove unmeasured confounding — only randomization or a natural experiment can","The sample was too small","Mortality cannot be measured","Propensity matching is invalid"], a:0,
      exp:"This is the key contrast with the Welsh shingles-vaccine study (L14), where an arbitrary date-of-birth cutoff mimicked randomization and DID support a causal claim."},
   ...
   },
   {
    ...
    slides:[
     {h:"Experiment 1 — ERCC1 knocked out only in blood cells (Yousefzadeh, Nature 2021)", imgs:[
        {src:"slides/L12/slide-13.jpg", cap:"L12 · Slide 13 — The paper: 'An aged immune system drives senescence and ageing of solid organs' (Nature 2021)"},{src:"slides/L12/slide-14.jpg", cap:"L12 · Slide 14 — ERCC1-KO(vav1): senescence markers in T cells and in non-immune tissues"},
  ...
  topics:[
   {
    ...
    slides:[
     ...
     {h:"Neuron structure and signaling", imgs:[
        {src:"slides/L13/slide-07.jpg", cap:"L13 · Slide 7 — Neurotransmitters and the states they influence"},
        {src:"slides/L13/slide-24.jpg", cap:"L13 · Slide 24 — Neuronal plasticity"}
      ], html:`
       <p class="lead"><b>The neurotransmitters — "they rule our lives":</b></p>
       <table class="dt">
        <tr><th>Neurotransmitter</th><th>Associated with</th></tr>
        <tr><td><b>Dopamine</b></td><td>Schizophrenia (high), anxiety, love; <b>and the neuron type lost in Parkinson's</b></td></tr>
        <tr><td><b>Serotonin</b></td><td>Happiness; <b>low in depression</b></td></tr>
        <tr><td><b>Oxytocin</b></td><td>Love / bonding</td></tr>
        <tr><td><b>Noradrenaline &amp; adrenaline</b></td><td>Fight or flight</td></tr>
       </table>
       <div class="callout tip">Depression is shown as <b>low dopamine AND low serotonin</b>; love as a combination of serotonin, dopamine and oxytocin. The Parkinson's link is the one that matters most here — the <b>dopaminergic</b> neurons of the midbrain are exactly the cells that die.</div>
  ...
  topics:[
   ...
   },
   {
    ...
    slides:[
     ...
     {h:"HER2 bispecifics — the OTHER kind of BMA", imgs:[], html:`
      <p class="lead">Blinatumomab is a <b>T-cell engager</b>, but that is not the only bispecific design. A <b>HER2 bispecific</b> illustrates the second type — and the distinction is very testable.</p>
      <table class="dt">
       <tr><th>Design</th><th>The two arms bind…</th><th>How it kills</th><th>Example</th></tr>
       <tr><td><b>T-cell engager (BiTE)</b></td><td>A <b>tumour antigen</b> + <b>CD3</b> on a T cell — <b>two different cells</b></td><td>Physically tethers a T cell to the cancer cell → T cell kills it</td><td><b>Blinatumomab</b> (CD19 × CD3)</td></tr>
       <tr><td><b>Biparatopic</b></td><td><b>Two different epitopes on the SAME antigen</b> — one cell</td><td>Cross-links HER2 receptors → clustering → strong internalization and degradation of HER2, plus enhanced immune effector recruitment</td><td><b>Zanidatamab</b> (HER2 × HER2)</td></tr>
      </table>
      <p><b>Zanidatamab mechanism:</b> because it grips <b>two separate sites on HER2</b> at once, it locks HER2 molecules together into clusters. That drives the receptor off the cell surface (internalization and degradation), shuts down HER2 growth signalling more completely than a single-epitope antibody like trastuzumab, and flags the cell for immune destruction (ADCC). It is used in <b>HER2-positive biliary tract, gastroesophageal and breast cancers</b>.</p>
      <div class="callout key"><b>The key contrast to hold:</b> a <b>BiTE bridges two different CELLS</b> (tumour + T cell). A <b>biparatopic bispecific binds two spots on ONE target</b>. Both are "bispecific monoclonal antibodies" — the exam-relevant question is <i>what are the two arms binding?</i></div>
      <div class="callout tip"><b>HER2 now appears in three different therapy classes</b> — a great integration point: <b>Herceptin/trastuzumab</b> = plain monoclonal antibody (L10) · <b>Enhertu</b> = ADC (antibody + topoisomerase I payload) · <b>Zanidatamab</b> = biparatopic bispecific. Same target, three drug modalities.</div>`},
    ...
    quiz:[
     ...
     {type:"mcq", q:"A bispecific antibody binds TWO DIFFERENT EPITOPES ON THE SAME antigen rather than bridging two cells. What is this design called, and give the HER2 example.", opts:["Biparatopic — zanidatamab (HER2 × HER2)","T-cell engager — blinatumomab","Antibody-drug conjugate — Enhertu","Checkpoint inhibitor — Keytruda"], a:0,
      exp:"Biparatopic = two arms, two epitopes, ONE antigen. Zanidatamab grips HER2 at two sites, clustering the receptor and driving its internalization and degradation."},
     {type:"mcq", q:"What is the fundamental difference between blinatumomab and zanidatamab, both of which are bispecific antibodies?", opts:["Blinatumomab bridges two different CELLS (CD19 tumour cell × CD3 T cell); zanidatamab binds two epitopes on ONE antigen (HER2 × HER2)","They are identical in design","Blinatumomab is an ADC","Zanidatamab engages T cells via CD3"], a:0,
      exp:"Both are 'bispecific', but ask what the arms bind: two cells (T-cell engager) vs two sites on one target (biparatopic)."},
     {type:"mcq", q:"How does binding HER2 at two separate epitopes make zanidatamab more effective than a single-epitope antibody?", opts:["It cross-links and clusters HER2, driving receptor internalization and degradation and shutting down HER2 signalling more completely","It delivers a chemotherapy payload","It blocks PD-1","It inserts a gene into T cells"], a:0,
      exp:"Receptor clustering pulls HER2 off the surface and enhances immune effector recruitment — more complete blockade than binding one site."},
     {type:"mcq", q:"HER2 is targeted by three different drug modalities covered in this course. Which set is correct?", opts:["Trastuzumab (plain monoclonal antibody), Enhertu (ADC), zanidatamab (biparatopic bispecific)","Keytruda, Provenge, Kymriah","Blinatumomab, Yescarta, Carvykti","Elahere, T-VEC, Libtayo"], a:0,
      exp:"Same target, three modalities: naked antibody blocks signalling; ADC adds a toxic payload; biparatopic bispecific clusters and degrades the receptor."},
